...
class LogisticRegression:
    ...
    def train(
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            batch_indexes = np.random.choice(a=y.size, size=batch_size, replace=False)
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        ###########################################################################
        predictions = self.activated_func(X * self.w)
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        h = self.activated_func(X_batch.dot(self.w))
        loss = -(np.dot(y_batch, np.log(h)) + np.dot(1 - y_batch, np.log(1 - h)))
        dw = (h - y_batch) * X_batch

        # Right now the loss is a sum over all training examples, but we want it
        # to be an average instead so we divide b   y num_train.
        # Note that the same thing must be done with gradient.
        num_train, _ = X_batch.shape
        loss = loss / num_train
        dw = dw / num_train
